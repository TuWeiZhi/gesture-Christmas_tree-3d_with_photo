# gesture-handling Specification

## Purpose

约束 `spring_festival.html` 中手势识别异常时的回退行为，避免摄像头丢失手势导致 STATE.mode 滞留在非默认状态（如 FIREWORK）。来源：`changes/lantern-firework-gesture-polish` 已实施并归档。
