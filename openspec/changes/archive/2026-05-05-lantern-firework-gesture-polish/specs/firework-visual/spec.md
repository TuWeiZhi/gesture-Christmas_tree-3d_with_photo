...
烟花爆炸粒子的初始速度范围 SHALL 从 `3 + random * 8` 增至 `4 + random * 12`。粒子基础尺寸 SHALL 增大 50%（`baseSizes` 乘以 1.5）。
...
爆炸后粒子拖尾（trail ghost）的参数 SHALL 调整：
- 存活时间：0.15s → 0.4s（`tAlp[i] - dt / 0.4` 替代 `dt / 0.15`）
- 尺寸系数：0.6× → 0.8×
- 亮度系数：50% → 70%
